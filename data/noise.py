...
"""
perlin/fractal noise adapted from
...
def generate_fractal_noise_3d(
        shape, res=(4, 4, 4), octaves=4, persistence=0.5, lacunarity=2,
    ...
    Args:
        shape: The shape of the generated array (tuple of three ints).
            This must [not] be a multiple of lacunarity**(octaves-1)*res.
        res: The number of periods of noise to generate along each
    ...
    old_shape = shape
    multof = [lacunarity**(octaves-1)*res[i] for i in range(3)]
    shape = [int(multof[i] * np.ceil(shape[i] / multof[i])) for i in range(3)]
    ...
    # Cut to correct size
    noise = noise[:old_shape[0], :old_shape[1], :old_shape[2]]

    # |perlin| <= sqrt(3/4) according to https://digitalfreepen.com/2017/06/20/range-perlin-noise.html
    # so |fraktal| <= 1/(1-q) * sqrt(3/4) with q = persistance < 1
    fraktal_range = 2 * 1 / (1 - persistence) * np.sqrt(3/4)
...
if __name__ == "__main__":
    noise = generate_fractal_noise_3d(shape=(100, 100, 100), res=(4, 4, 4), octaves=4)
